...
class TestInitCommand(BaseClass):
    ...
    def test_it_initializes_a_follower_git_repo(self):
        self.assertIsNotDir(ET_FOLLOWER_ROOT_DIR, self.test_proj_name)
        init(self.source_proj_path)
        self.assertIsDir(ET_FOLLOWER_ROOT_DIR, self.test_proj_name)
        self.assertIsDir(ET_FOLLOWER_ROOT_DIR, self.test_proj_name, '.git')
